...
// total result and  checking if the election was rigged or not:
app.get("/gettotal", async (req, res) => {
  const { parties } = req.body;
  const checkRig = await electionModel.findOne({ parties });

  const getName = (await electionModel.find())
    .filter((el) => el.parties === parties)
    .map((el) => el.parties)[0];
...
app.post("/post-election", async (req, res) => {
  const data = req.body;
...
app.put("/rigged/:stateId", async (req, res) => {
  const stateId = req.params.stateId;
...
app.delete("/results/:stateId", async (req, res) => {
  const stateId = req.params.stateId;
  const data = req.body;
  const updatedResult = await electionModel.findByIdAndDelete(stateId, data, {
    new: true,
  });
  if (!updatedResult) {
    res.status(400).json({
